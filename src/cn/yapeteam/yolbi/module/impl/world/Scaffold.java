...
public class Scaffold extends Module {
    ...
    private final NumberValue pitchValue = new NumberValue("Pitch", () -> rotationsMode.is("Movement based"), 81.5, 70, 110, 0.5);

    private final ModeValue noSprintTiming = new ModeValue("No sprint timing", "Always", "Always", "When rotating", "Never");
    private final ModeValue noSprintMode = new ModeValue("No sprint", () -> !noSprintTiming.is("Never"), "Normal", "Normal", "Spoof");

    private final ModeValue raytrace = new ModeValue("Raytrace", "Disabled", "Disabled", "Normal", "Legit");
    private final NumberValue negativeExpand = new NumberValue("Negative expand", 0, 0, 0.24, 0.01);
    private final NumberValue offGroundNegativeExpand = new NumberValue("Offground negative expand", 0, 0, 0.24, 0.01);

    private final NumberValue delayBetweenPlacements = new NumberValue("Delay between placements", 0, 0, 10, 1);
    ...
    private final ModeValue offGroundStrafe = new ModeValue("Off ground strafe", () -> strafe.getValue() && !moveFix.getValue(), "Disabled", "Disabled", "Enabled", "Keep movement");
    private final NumberValue strafeSpeed = new NumberValue("Speed", () -> strafe.getValue() && !moveFix.getValue(), 0.2, 0.1, 0.5, 0.005);
    private final NumberValue overAirSpeed = new NumberValue("Over air speed", () -> strafe.getValue() && !moveFix.getValue(), 0.1, 0, 0.5, 0.005);
    private final NumberValue offGroundSpeed = new NumberValue("Offground speed", () -> strafe.getValue() && !offGroundStrafe.is("Disabled") && !moveFix.getValue(), 0.2, 0.1, 0.5, 0.005);
    private final NumberValue strafeSpeedPotExtra = new NumberValue("Speed pot extra", () -> strafe.getValue() && !moveFix.getValue(), 0.2, 0, 0.2, 0.005);
    private final BooleanValue randomisedSpeed = new BooleanValue("Randomised speed", () -> strafe.getValue() && !moveFix.getValue(), false);

    private final ModeValue jump = new ModeValue("Jump", "Disabled", "Disabled", "Enabled", "Hypixel");

    private final NumberValue range = new NumberValue("Range", 2, 1, 4, 1);

    private final ModeValue tower = new ModeValue("Tower", "None", "Vanilla", "NCP", "NCP2", "OldHypixel", "MMC", "None");

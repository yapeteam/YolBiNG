...
@SuppressWarnings({"UnnecessaryUnicodeEscape", "DuplicatedCode", "unused"})
public class UnicodeFontRenderer extends FontRenderer implements AbstractFontRenderer {
    private final UnicodeFont font;

    // TODO: 2021/5/20 中文字体加载速度过于拉胯 待修复 提供几个解决方法:1.使用Dll渲染中文，参考csgo的opengl注入外纪(老嘎嘎说的，可行?) 2.获取字体资源，用哪个字加载哪个字
    // 其实我的世界用dll渲染纯属b
    public UnicodeFontRenderer(Font awtFont, int startCodePoint, int endCodePoint, boolean b) {
        ...
        if (endCodePoint > -1 && startCodePoint > -1) {
    ...
    public float drawOutlinedString(String str, float x, float y, int color) {
    ...
    public float drawString(String text, float x, float y, boolean always) {
        if (text == null) {
            return 0;
        } else {
            text = "\u00a7r" + text + "\u00a7r";
    ...
    @Override
    public float drawString(String text, float x, float y, int color, boolean dorpshadow) {
        int ColorBak = color;
